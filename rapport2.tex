...
\newpage
 ...
 \begin{section}{Dispositif expérimental}
   ...
   \begin{subsubsection}{Enceinte à vide}
    Dans l'einceinte à vide se trouve le détecteur et un sélecteur de source pour que nous n'ayons pas dû toucher les sources nues $\alpha$ se qui est très dangereux et peut causé de cancer. En plus il y a une pompe pour faire le vide et un système pour faire rentrer l'air précisement. Un jauge capacitifcalibré un jauge Pirani très precis mesure la pression dans l'enceinte avec une précision de \unit[0,2]{\%}.  
   \end{subsubsection}

   \begin{subsubsection}{Détecteur et préamplificateur}
    \todo{ev. Bild finden}
    Le détecteur consiste d'une jonction Si(Li) avec une barrière de surface et le zone active du détecteur est réglée par la tension de polarisation. La jonction est assimilable à un condensateur plan dont la distance est égale à la zone désertée (=active?) \todo{Problem verstehen, dass durch den preamplificateur behoben wird}. 
    Le préamplificateur de charge est rélie à un module de mise en forme qui fournit un signal de l'amplitude proportionnel à la charge collecté dans la jonction du détecteur.

    source de haute tension pour la dépolarisation du détecteur
   \end{subsubsection}

   \begin{subsubsection}{Logiciel}
    Un amplificateur entre le préamplificateur et l'analysateur d'amplitude donne la possibilité d'ajuster le signal. L'analyseur d'amplitude est rélie à l'ordinateur où un logiciel met le signal en fonction de on ne sait pas.
   \end{subsubsection}
  \end{section}
  
  \begin{section}{Expériences préliminaire}
  \begin{subsection}{Etude de la réponse du détecteur}
   \todo{im Anhang nachschauen}
   Pour comprendre le comportement du détecteur en fonction de la tension, nous avon fait...
  \end{subsection} 
